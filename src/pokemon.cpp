#include "pokemon.h" //we need to know what class we're building!
#include "fastMove.h"//pokemon.h already includes these class fiels for us, they're just added here for clarity.
...
pokemon::pokemon(string Name, int Level, int type1, int atk, int sta, int def, fastMove Fast, chargedMove Charged) { //option for a pokemon with just one type
    srand(time(NULL));
    name = Name;
...
pokemon::pokemon(string Name,  int Level, int type1, int type2, int atk, int sta, int def,  fastMove Fast, chargedMove Charged) { // option for a pokemon with two types
