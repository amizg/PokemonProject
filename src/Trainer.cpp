...
void trainer::chooseBattleRoster() {
    ...
    cout << endl << name << ", the pokemon on your roster are " << roster[battleRoster[0]].getName() << ", " << roster[battleRoster[1]].getName() << ", and " << roster[battleRoster[2]].getName() << "!" << endl << endl;
}
...
float trainer::getCooldown(/*fastMove& fastMove*/) const{
    return roster[battleRoster[activePoke]].getFastCD();//fastMove.getFastMoveCoolDown();
};
float trainer::getDuration(/*fastMove& fastMove*/) const{
    return roster[battleRoster[activePoke]].getDuration();//fastMove.getFastMoveCoolDown();
};
void trainer::changeWaitTime(float num) {
    waitTime += num;
    if (waitTime < 0.1){waitTime = 0;}
};

void trainer::decWait(){
    waitTime -= 0.5;
    if (waitTime < 0.1) {waitTime = 0;}

    cout << "------------Current Wait Time: " << waitTime  << " s" << endl  << endl;
}
...
void trainer::fastAttack(trainer & enemy) {
    ...
    cout << endl << name << "'s " << roster[battleRoster[activePoke]].getName() << " used " << roster[battleRoster[activePoke]].getFastName() << "!" << endl;
  ...
  /*  for(int i = 0; i < (enemy.getDuration() * 2) - 1; i++) {
    ...
    */
    ...
    int totalDamage;
    if (enemy.roster[enemy.battleRoster[enemy.activePoke]].checkTwoTypes()) //if the enemy pokemon has two types,
    {
    ...
    {
        cout << endl << "It was super effective!" << endl;
    }
    if ((roster[battleRoster[activePoke]].refTypeMatchup(roster[battleRoster[activePoke]].getFastType(), enemy.roster[enemy.battleRoster[enemy.activePoke]].getType(0)) * roster[battleRoster[activePoke]].refTypeMatchup(roster[battleRoster[activePoke]].getFastType(), enemy.roster[enemy.battleRoster[enemy.activePoke]].getType(1)) * roster[battleRoster[activePoke]].chargedResonanceCheck()) < 1)
    {
        cout << endl << "It was not very effective!" << endl;
    }
    }
    else {
    ...
    cout << endl << enemy.roster[enemy.battleRoster[enemy.activePoke]].getName() << " has taken " << totalDamage << " damage, and is now at " << enemy.roster[enemy.battleRoster[enemy.activePoke]].getCurrentHP() << " HP!" << endl << endl;
    }
    else {
        cout << endl << enemy.roster[enemy.battleRoster[enemy.activePoke]].getName() << " has taken " << totalDamage << " damage. " << endl;
   ...
   // if (!(enemy.notDead()))
    ...
    changeWaitTime(getCooldown() + getDuration());
}
void trainer::chargedAttack(trainer & enemy) {
    ...
    if (roster[battleRoster[activePoke]].getEnergy() >= roster[battleRoster[activePoke]].getChargedCost()) {
        ...
        if (enemy.getShields() > 0) {
            ...
            cout << endl << "Incoming Charge Attack!" << endl;
        ...
        //if they don't, huzzah! attack!
            ...
            if (enemy.roster[enemy.battleRoster[enemy.activePoke]].getCurrentHP() > 0) {
                ...
                cout << endl << enemy.roster[enemy.battleRoster[enemy.activePoke]].getName() << " has taken " << totalDamage << " damage. " << endl;
                cout << endl << enemy.roster[enemy.battleRoster[enemy.activePoke]].getName() << " has feinted!" << endl << endl;
            }
            roster[battleRoster[activePoke]].changeEnergy(-1 * (roster[battleRoster[activePoke]].getChargedCost()));
            if (enemy.roster[enemy.battleRoster[enemy.activePoke]].getCurrentHP() <= 0 && enemy.notDead())
                enemy.chooseActive();
...
void trainer::turnAgainst(trainer& enemy) {
    ...
    while (wrong) {
        wrong = false;
        cout << endl << "It is " << name << "'s turn! Choose your action:" << endl;
        cout << endl << " | " << roster[battleRoster[activePoke]].getName() << " | " << roster[battleRoster[activePoke]].getCurrentHP()
            << " HP | " << " Charge Energy "<<" [" << roster[battleRoster[activePoke]].getEnergy()
               << "/" << roster[battleRoster[activePoke]].getChargedCost() << "] |" << endl;
